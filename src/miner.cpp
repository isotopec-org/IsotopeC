    ...
    {   
        ...
        *phash = block.GetHashYespower();
        ...
        if ((nNonce & 0xfff) == 0 || miner->fKill > 0) {
            if (miner->fKill > 0)
                LogPrintf("IsotopeC miner kill flag > 0\n");
            return false;
        }
    }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash()) {
            LogPrintf ("IsotopeC Miner: generated block is stale\n");
            return false;
        }
    }
    ...
    if (!ProcessNewBlock (chainparams, ptr, true, nullptr)) {
        LogPrintf ("IsotopeC Miner: ProcessNewBlock, block not accepted\n");
        result =  false;
    }
...
void static ISOMiner(MinerInfo* miner, const CChainParams& chainparams)
{
    LogPrintf("IsotopeC Miner started\n");
    SetThreadPriority (THREAD_PRIORITY_LOWEST);
    RenameThread ("isotopec-miner");
    ...
    try {
        while (true) {
            ...
            {
                LogPrintf("IsotopeC Miner runtime error: %s\n", e.what());
                LogPrintf("IsotopeC Miner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                MilliSleep(4*1000);
                vpwallets[0]->GetScriptForMining (coinbaseScript);
                continue;
            }
            ...
            LogPrintf("Running IsotopeC Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true) {
                ...
                if (ScanHash (miner, pblock, nNonce, &hash))
                {
                    if (UintToArith256(hash) <= hashTarget)
                    {
                        ...
                        LogPrintf("IsotopeC Miner: proof-of-work found  \n  hash: %s  \ntarget: %s\n",
    ...
    {
        LogPrintf("IsotopeC Miner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
...
void GenerateISO(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ISOMiner, vMiners[i], boost::cref(chainparams)));
