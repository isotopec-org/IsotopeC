...
#if defined(NDEBUG)
# error "IsotopeC cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "IsotopeC Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Force block reward to zero when right shift is undefined at 36 intervals.
    if (halvings >= 36)
        return 0;

    CAmount nSubsidy = 32 * COIN;
    // Subsidy is cut in half every 420,480 blocks which will occur approximately every 4 years.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(IsYesPower(nHeight) ? block.GetHashYespower() : block.GetHash(), block.nBits, consensusParams))
