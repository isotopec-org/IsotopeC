...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount && 
                !CheckProofOfWork(IsYesPower(nHeight) ? pblock->GetHashYespower() : pblock->GetHash(), pblock->nBits, Params().GetConsensus())) 
        {
            ++pblock->nNonce;
            --nMaxTries;
        }
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated IsotopeC to.\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    GenerateISO (request.params[0].get_bool(), numCpus, Params());
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "IsotopeC is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "IsotopeC is downloading blocks...");
